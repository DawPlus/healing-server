// 열 번째 시트 - 효과성분석(상담치유효과)
export const createSheet10 = async (workbook, data, defaultStyle, headerStyle) => {
  const worksheet = workbook.addWorksheet('효과성분석(상담치유효과)');
  ...
  const programEffectSubHeaders = ['상담치유효과(합계)', '상담치유효과(평균)'];
  ...
  if (data && data.length > 0) {
    data.forEach(item => {
      ...
      sheet10Data.push([
        ...
        item.counseling_therapy_total || item.상담치유효과합계 || 0,
        item.counseling_therapy_average || item.상담치유효과평균 || 0
      ]);
    });
  }
  
  // 데이터 추가
  if (sheet10Data.length > 0) {
    sheet10Data.forEach((row, index) => {
  ...
  worksheet.columns = [
    ...
    { width: 10 },  // F: 참가인원
    { width: 12 },  // G: 구분
    { width: 18 },  // H: 상담치유효과(합계)
    { width: 18 }   // I: 상담치유효과(평균)
  ];
