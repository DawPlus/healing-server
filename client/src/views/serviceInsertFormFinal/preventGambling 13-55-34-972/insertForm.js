    ...
    useImperativeHandle(ref, () => ({
        ...
        setRows: (newRows) => {
            if (setAllData && typeof setAllData === 'function') {
                console.log("[PreventGambling InsertForm] 🔄 setRows 호출됨:", newRows.length);
                console.log("[PreventGambling InsertForm] 🔍 참가자 데이터 첫 번째 행:", 
                    JSON.stringify(newRows[0]).substring(0, 200) + '...');
                
                // 상위 컴포넌트의 setAllData 메서드를 사용하여 행 업데이트
                console.log("[PreventGambling InsertForm] 🔄 setAllData 호출 시작");
                setAllData({type: 'all', value: newRows});
                console.log("[PreventGambling InsertForm] ✅ setAllData 호출 완료");
                return true;
            }
            console.log("[PreventGambling InsertForm] ❌ setAllData 함수 없음");
            return false;
        },
        // 현재 rows 노출
        getRows: () => rows
