...
import { GET_PAGE5_RESERVATION_DETAIL, GET_PAGE5_RESERVATION_LIST } from '../Page5/graphql/queries';
import { formatDate } from '../Page5/services/dataService';
import {
  Box,
  Typography,
  Grid,
...
import { exportToExcel } from '../Page5/inspection/excelExport'; // 엑셀 내보내기 함수 임포트
import { exportProgramSchedulePdf } from '../Page5/inspection/programSchedulePdf'; // 프로그램 일정표 PDF 내보내기 함수 임포트
import { exportRoomMealPdf } from '../Page5/inspection/roomMealPdf'; // 숙소배정 및 식사인원 PDF 내보내기 함수 임포트
import { exportCustomerUsageExcel } from '../Page5/inspection/customerUsageExport'; // 고객이용내역서 엑셀 내보내기 함수 임포트
import { exportUsageReportPdf } from '../Page5/inspection/usageReportPdf'; // 이용내역서 PDF 내보내기 함수 임포트
import { exportCustomerUsagePdf } from '../Page5/inspection/customerUsagePdf'; // 고객이용내역서 PDF 내보내기 함수 임포트
...
const UsageReportTab = () => {
  ...
  const handleExcelExport = async () => {
    ...
    fetchPolicy: 'network-only'
      ...
      if (!detailData) {
        throw new Error('이용내역서 상세 정보를 불러올 수 없습니다.');
      }
  ...
  const handleCustomerUsageExport = async () => {
    ...
    try {
      ...
      console.error('Customer usage excel export error:', error);
      Swal.fire({
        icon: 'error',
        title: '엑셀 다운로드 실패',
        text: error.message || '엑셀 파일 생성 중 오류가 발생했습니다.'
  ...
  // 이용내역서 PDF 내보내기 핸들러 (기존 견적서 PDF 로직 사용)
  const handleInvoicePdf = async () => {
    ...
    try {
      ...
      // PDF 내보내기 함수 호출
      const success = await exportUsageReportPdf(detailData);

    if (success) {
        Swal.fire({
          icon: 'success',
          title: 'PDF 다운로드 성공',
          text: '이용내역서가 PDF 파일로 다운로드 되었습니다.'
        });
    } else {
            ...
            <CardHeader 
              title={
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Typography variant="h6">이용내역서 목록</Typography>
            </Box>
              }
              action={
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Grid container spacing={2}>
            ...
            <Button
              fullWidth
                      ...
                      >
                        {invoicePdfLoading ? <CircularProgress size={24} /> : '이용내역서 PDF'}
            </Button>
          </Grid>
        </Grid>
                </CardContent>
                ...
                <CardHeader 
                  title={
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Typography variant="h6">이용내역서 미리보기</Typography>
                    </Box>
                  }
                />
                <Divider />
        <CardContent>
                  {selectedDoc ? (
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '300px' }}>
                      <Typography color="textSecondary">이용내역서를 선택하세요</Typography>
            </Box>
                  )}
                </CardContent>
              </Card>
...
const DocumentPreview = ({ data }) => {
  ...
  const calculateTotal = () => {
    ...
    roomSelections.forEach(room => {
      console.log('UsageReportTab calculateTotal - 객실 정보:', room);
      ...
      if (room.total_price && parseInt(room.total_price) > 0) {
        ...
        console.log('UsageReportTab calculateTotal - total_price(수동입력) 사용:', subtotal);
      } else {
        // page3와 동일한 계산 공식 적용:
        ...
        if (occupancy > capacity) {
          ...
          console.log(`UsageReportTab calculateTotal - 인원수 초과 요금: ${extraPeople}명 × 10,000원 × ${nights}박 = ${extraCharge}원`);
        }
        
        console.log('UsageReportTab calculateTotal - page3 공식으로 계산된 가격:', {
          객실타입: room.room_type,
          기본가격: baseRoomPrice,
          박수: nights,
          투숙인원: occupancy,
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {groupedRooms.length > 0 ? groupedRooms.map((group, index) => {
              console.log('UsageReportTab DocumentPreview - 그룹화된 객실 정보:', group);
      ...
      <Box sx={{ marginBottom: '10px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            <tr>
              <td style={headerCellStyle} colSpan={4}>소 계</td>
              <td style={dataCellStyle}>₩ {totals.etcTotal.toLocaleString()}</td>
              <td style={dataCellStyle}></td>
            </tr>
          </tbody>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <tbody>
          <tr>
            ...
            <td style={{ ...dataCellStyle, border: 'none', width: '25%' }}></td>
            <td style={{ ...dataCellStyle, textAlign: 'left', border: 'none', width: '25%' }}>
              회 사 명 : {detail.group_name || '-'}
            </td>
          </tr>
          <tr>
            <td style={{ ...dataCellStyle, textAlign: 'left', border: 'none' }}>
              담 당 자 : {detail.reservation_manager || '김재훈'}
            </td>
            <td style={{ ...dataCellStyle, border: 'none' }}></td>
            <td style={{ ...dataCellStyle, textAlign: 'left', border: 'none' }}>
