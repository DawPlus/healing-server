...
/**
 * Export invoice as PDF with exact layout matching DocumentPreview component
 * @param {Object} reservationData - The reservation data
 * @returns {boolean} - Whether the export was successful
 */
export const exportInvoicePdf = async (reservationData) => {
  try {
    ...
    // Calculate totals with discount rates (same as DocumentPreview)
    ...
    let venueTotal = 0;
      ...
      // Get discount rate function (same as DocumentPreview)
      ...
      // Room calculation - page3 completeRoomSave 함수와 동일한 로직 적용
      const roomDiscountRate = getDiscountRate('room');
      roomSelections.forEach(room => {
        console.log('invoicePdf calculateTotal - 객실 정보:', room);
        ...
        if (room.total_price && parseInt(room.total_price) > 0) {
          ...
          console.log('invoicePdf calculateTotal - total_price(수동입력) 사용:', roomTotalPrice);
        } else {
          // page3와 동일한 계산 공식 적용:
          ...
          if (occupancy > capacity) {
            ...
            console.log(`invoicePdf calculateTotal - 인원수 초과 요금: ${extraPeople}명 × 10,000원 × ${nights}박 = ${extraCharge}원`);
          }
          
          console.log('invoicePdf calculateTotal - page3 공식으로 계산된 가격:', {
            객실타입: room.room_type,
            기본가격: baseRoomPrice,
            박수: nights,
            투숙인원: occupancy,
      ...
      // Meal calculation (same as DocumentPreview)
      ...
      // Program calculation (same as DocumentPreview)
      ...
      // Etc items calculation (same as DocumentPreview)
      const etcDiscountRate = getDiscountRate('etc');
      
      if (pageFinal && pageFinal.income_items) {
        // Material costs from income items
        ...
        materialIncomes.forEach(income => {
          ...
          const discountRate = income.discount_rate !== null && income.discount_rate !== undefined ? income.discount_rate : etcDiscountRate;
          const discount = Math.round(amount * (discountRate / 100));
          const total = amount - discount;
          etcTotal += total;
          
          etcItems.push({
            name: income.details || income.category || '재료비',
        ...
        etcIncomes.forEach(income => {
          ...
          etcItems.push({
            name: income.details || income.category || '기타비',
            material_type: 'income',
            amount: amount,
            discount: discount,
            total: total,
      ...
      // Page4 material costs as fallback (same as DocumentPreview)
    ...
    // Meal type translations (same as DocumentPreview)
    ...
    // Create HTML content with exact same styling as DocumentPreview
    const htmlContent = createInvoiceHTML(detail, totals, totalAttendees, typeTranslations, roomSelections, mealPlans, programs, placeReservations);
    ...
    try {
      ...
      // Save PDF
      const fileName = `${detail.group_name || '견적서'}_${moment().format('YYYYMMDD')}.pdf`;
      pdf.save(fileName);
...
// Function to create HTML content matching DocumentPreview exactly
function createInvoiceHTML(detail, totals, totalAttendees, typeTranslations, roomSelections, mealPlans, programs, placeReservations) {
  // Common styles (same as DocumentPreview)
  ...
  return `
    <div style="width: 100%; max-width: 210mm; margin: 0 auto; padding: 20px; font-family: 'Malgun Gothic', '맑은 고딕', sans-serif; font-size: 10px; line-height: 1.2; background-color: white;">
      ...
      <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 10px; border: 2px solid #000; padding: 8px;">
        <div style="font-size: 18px; font-weight: bold;">단체행사 견적/계약서</div>
      </div>
      
      <!-- 안내문구 -->
      <div style="text-align: center; margin-bottom: 10px; border: 1px solid #000; padding: 8px; font-size: 10px;">
        <div>하이힐링원에 관심을 기울여 주심에 감사드리며 아래와 같이 견적서를 제출합니다.</div>
      ...
      <table style="width: 100%; border-collapse: collapse;">
        <tbody>
          <tr>
            ...
            <td style="text-align: right; border: none; width: 50%;">회 사 명 : ${detail.group_name || '-'}</td>
          </tr>
          <tr>
            <td style="text-align: left; border: none; width: 50%;">담 당 자 : ${detail.reservation_manager || '김재훈'}</td>
            <td style="text-align: right; border: none; width: 50%;">담 당 자 : ${detail.customer_name || '-'}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(인)</td>
          </tr>
        </tbody>
      </table>
    </div>
...
function getDiscountText(discountRates) {
  const discountTexts = [];
  if (discountRates.room > 0) discountTexts.push(`객실 ${discountRates.room}%`);
  if (discountRates.meal > 0) discountTexts.push(`식사 ${discountRates.meal}%`);
  if (discountRates.program > 0) discountTexts.push(`프로그램 ${discountRates.program}%`);
  if (discountRates.etc > 0) discountTexts.push(`기타 ${discountRates.etc}%`);
  
  return discountTexts.length > 0 ? `(${discountTexts.join(', ')})` : '';
}
...
function createRoomSection(roomSelections, totals, headerCellStyle, dataCellStyle) {
  ...
  if (roomSelections.length > 0) {
    ...
    groupedRooms.forEach((group) => {
      console.log('invoicePdf createRoomSection - 그룹화된 객실 정보:', group);
