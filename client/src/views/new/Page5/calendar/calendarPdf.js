...
/**
 * Export calendar schedule as PDF
 ...
 */
export const exportCalendarPdf = (calendarData, currentDate) => {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      dayEvents.forEach((event, index) => {
        if (index < 3) { // Show max 3 events per day for better readability
          ...
          if (event.status === '예약확인' || event.reservation_status === '예약확인') {
