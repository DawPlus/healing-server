...
const getYearMonthResults = async (_, { startDate, endDate, keywords }) => {
  ...
  try {
    ...
    // 체류기간 검색이 있는지 확인
    let daysToStayFilter = null;
    if (keywords) {
      const daysKeyword = keywords.find(k => k.type === 'DAYS_TO_STAY');
      if (daysKeyword && daysKeyword.value) {
        daysToStayFilter = parseInt(daysKeyword.value);
        // 체류기간 검색은 별도로 처리하므로 키워드에서 제거
        keywords = keywords.filter(k => k.type !== 'DAYS_TO_STAY');
      }
    }

    // 일반 키워드 검색 조건 추가 (체류기간 제외)
    ...
    // 쿼리 실행
    let page2Data = await prisma.page2.findMany({
      where: whereCondition,
      include: {
        page1: {
          include: {
            page3: true // page3 데이터 포함
          }
        }
      }
    });

    // 체류기간 필터링 (클라이언트 측에서 처리)
    if (daysToStayFilter && !isNaN(daysToStayFilter)) {
      console.log(`체류기간 필터링: ${daysToStayFilter}일`);
      page2Data = page2Data.filter(item => {
        if (!item.page1?.start_date || !item.page1?.end_date) {
          return false;
        }
        
        const startDate = new Date(item.page1.start_date);
        const endDate = new Date(item.page1.end_date);
        
        // 체류일수 계산 (시작일 포함)
        const diffTime = endDate.getTime() - startDate.getTime();
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1;
        
        return diffDays === daysToStayFilter;
      });
      console.log(`체류기간 필터링 후 결과: ${page2Data.length}건`);
    }
    ...
    console.log(`최종 검색 결과: ${page2Data.length}건`);
    
    // 결과 데이터 변환
    return page2Data.map(item => {
      // Page3의 room_selections에서 스탠다드룸과 디럭스룸 개수 계산
      let standardRoomCount = 0;
      let deluxeRoomCount = 0;
      let totalRoomCount = 0;
      
      if (item.page1?.page3?.room_selections) {
        try {
          const roomSelections = typeof item.page1.page3.room_selections === 'string' 
            ? JSON.parse(item.page1.page3.room_selections) 
            : item.page1.page3.room_selections;
          
          if (Array.isArray(roomSelections)) {
            // 각 객실을 개별적으로 카운트
            const roomData = roomSelections.reduce((acc, room) => {
              if (room.room_type === '디럭스') {
                acc.deluxe += 1;
              } else {
                acc.standard += 1; // 디럭스가 아닌 모든 객실은 스탠다드로 분류
              }
              return acc;
            }, { standard: 0, deluxe: 0 });
            
            standardRoomCount = roomData.standard;
            deluxeRoomCount = roomData.deluxe;
            totalRoomCount = standardRoomCount + deluxeRoomCount;
          }
        } catch (error) {
          console.error('Error parsing room_selections:', error);
          // fallback to basic room count
          const participantRoomCount = Math.ceil((item.male_count + item.female_count) / 4);
          const leaderRoomCount = Math.ceil((item.male_leader_count + item.female_leader_count) / 2);
          totalRoomCount = participantRoomCount + leaderRoomCount;
        }
      } else {
        // fallback to basic room count if no room_selections data
        const participantRoomCount = Math.ceil((item.male_count + item.female_count) / 4);
        const leaderRoomCount = Math.ceil((item.male_leader_count + item.female_leader_count) / 2);
        totalRoomCount = participantRoomCount + leaderRoomCount;
      }
      ...
      // Include region directly for ResidenceList component
        ...
        // Include meal_plans data
        meal_plans: item.page1?.page3?.meal_plans ? (
          typeof item.page1.page3.meal_plans === 'string' 
            ? item.page1.page3.meal_plans 
            : JSON.stringify(item.page1.page3.meal_plans)
        ) : null,
        // Include room data
...
const calculateHealingEffect = async (startDate, endDate, keywords) => {
  let where = {
    openday: {
      gte: startDate,
      lte: endDate,
  ...
  const healingForms = await prisma.healingForm.findMany({ where });

  const R = healingForms.reduce((acc, cur) => {
    const {
    ...
    acc[pv].physicalFitness.push(calculateAverage([score1, score2]));
    acc[pv].physicalPositive.push(calculateAverage([score3, score4, score5]));
    acc[pv].physicalSleep.push(calculateAverage([score6, score7, score8, score9]));
    acc[pv].psychologicalAnxiety.push(calculateAverage([score10, score11, score12]));
    acc[pv].psychologicalDepression.push(calculateAverage([score13, score14, score15, score16]));
    acc[pv].psychologicalSelfEsteem.push(calculateAverage([score17, score18, score19]));
    acc[pv].psychologicalStress.push(calculateAverage([score20, score21, score22]));
  ...
  for (const pv in R) {
    for (const category in R[pv]) {
      A[pv][category] = calculateAverage(R[pv][category]);
    }
  }
...
const getYearMonthServiceStats = async (_, { startDate, endDate, keywords }) => {
  try {
    ...
    // 연령대 매핑
    const ageTypeMapping = {
      '아동청소년': 'kidboy', '성인': 'adult', '노인': 'old'
    };
    
    // 사업구분 매핑
    const businessCategoryMapping = {
      'social_contribution': 'society', '사회공헌': 'society',
      'profit_business': 'benefit', '수익사업': 'benefit'
    };
    
    // 참여형태 매핑 (단체, 개인, 기타 -> count_income_etc, count_income_green, count_income_voucher)
    const partFormMapping = {
      '단체': 'income_etc', '개인': 'income_green', '기타': 'income_voucher'
    };
    ...
    page2Data.forEach(item => {
      ...
      const ageType = item.age_type;
      const businessCategory = item.page1?.business_category;
      const partForm = item.part_form;
      ...
      // 연령대 처리
      const mappedAgeKey = ageTypeMapping[ageType];
      if (mappedAgeKey) {
          partTypeListResult[`part_${mappedAgeKey}`] = (partTypeListResult[`part_${mappedAgeKey}`] || 0) + participants;
          partTypeListResult[`count_${mappedAgeKey}`] = (partTypeListResult[`count_${mappedAgeKey}`] || 0) + 1;
      }
      
      // 사업구분 처리
      const mappedBusinessKey = businessCategoryMapping[businessCategory];
      if (mappedBusinessKey) {
          partTypeListResult[`part_${mappedBusinessKey}`] = (partTypeListResult[`part_${mappedBusinessKey}`] || 0) + participants;
          partTypeListResult[`count_${mappedBusinessKey}`] = (partTypeListResult[`count_${mappedBusinessKey}`] || 0) + 1;
      }
      
      // 참여형태 처리 (단체, 개인, 기타)
      const mappedPartFormKey = partFormMapping[partForm];
      if (mappedPartFormKey) {
          partTypeListResult[`part_${mappedPartFormKey}`] = (partTypeListResult[`part_${mappedPartFormKey}`] || 0) + participants;
          partTypeListResult[`count_${mappedPartFormKey}`] = (partTypeListResult[`count_${mappedPartFormKey}`] || 0) + 1;
      }
    ...
    console.log('Final partTypeListResult:', JSON.stringify(partTypeListResult, null, 2));
    ...
    // --- Step 4: Calculate New Sections Data ---
    console.log('Calculating new sections data...');
    
    // Initialize results
    const preventEffectResult = {
      smartphone: {
        preventSum: 0, preventAvg: 0, counselSum: 0, counselAvg: 0,
        healingTotalSum: 0, healingAverageScore: 0,
        avgScore1: 0, avgScore2: 0, avgScore3: 0, avgScore4: 0, avgScore5: 0, avgScore6: 0
      },
      gambling: {
        preventSum: 0, preventAvg: 0, counselSum: 0, counselAvg: 0,
        healingTotalSum: 0, healingAverageScore: 0,
    ...
    const facilityEnvironmentResult = {
      facility_avg1: 0, facility_avg2: 0, facility_avg3: 0, facility_avg4: 0, facility_avg5: 0,
      facility_avg6: 0, facility_avg7: 0, facility_avg8: 0, facility_avg9: 0, facility_avg10: 0,
      operation_avg1: 0, operation_avg2: 0, operation_avg3: 0, operation_avg4: 0, operation_avg5: 0,
      operation_avg6: 0, overall_satisfaction: 0
    };

    const healingEffectResult = {
      pre_avg1: 0, pre_avg2: 0, pre_avg3: 0, pre_avg4: 0, pre_avg5: 0, pre_avg6: 0, pre_avg7: 0,
      post_avg1: 0, post_avg2: 0, post_avg3: 0, post_avg4: 0, post_avg5: 0, post_avg6: 0, post_avg7: 0,
      diff_avg1: 0, diff_avg2: 0, diff_avg3: 0, diff_avg4: 0, diff_avg5: 0, diff_avg6: 0, diff_avg7: 0,
      experienceStats: {
        hasExperience: 0, // 유
        noExperience: 0, // 무
        notSpecified: 0 // 미기재
    ...
    try {
      // 1. Prevent Effect Data (스마트폰/도박 예방효과)
      console.log('디버깅중입니다 - Prevent Effect 데이터 조회 시작');
      console.log('디버깅중입니다 - 날짜 범위:', { startDate, endDate });
      
      const preventFormsData = await prisma.preventForm.findMany({
        where: {
          openday: { gte: startDate, lte: endDate }
        }
      });

      const preventGamblingFormsData = await prisma.preventGamblingForm.findMany({
        where: {
          openday: { gte: startDate, lte: endDate }
        }
      });

      console.log(`디버깅중입니다 - Prevent Forms 조회 결과: ${preventFormsData.length}개`);
      console.log(`디버깅중입니다 - Prevent Gambling Forms 조회 결과: ${preventGamblingFormsData.length}개`);
      
      if (preventFormsData.length > 0) {
        console.log('디버깅중입니다 - 첫 번째 Prevent Form 데이터:', preventFormsData[0]);
      }
      if (preventGamblingFormsData.length > 0) {
        console.log('디버깅중입니다 - 첫 번째 Prevent Gambling Form 데이터:', preventGamblingFormsData[0]);
      }

      // Process smartphone prevention data (prevent_forms)
      if (preventFormsData.length > 0) {
        const smartphonePreData = preventFormsData.filter(form => form.pv === '사전');
        const smartphonePostData = preventFormsData.filter(form => form.pv === '사후');

        console.log(`디버깅중입니다 - 스마트폰 사전 데이터: ${smartphonePreData.length}개`);
        console.log(`디버깅중입니다 - 스마트폰 사후 데이터: ${smartphonePostData.length}개`);

        // Calculate smartphone scores (scores 1-3 for 중독특징이해, 4-6 for 핵심증상이해, etc.)
        const calculateScoreAverage = (forms, scoreRange) => {
          console.log(`디버깅중입니다 - 점수 계산 중: ${forms.length}개 폼, 점수 범위 ${scoreRange.start}-${scoreRange.end}`);
          ...
          forms.forEach((form, idx) => {
            let formSum = 0;
            let formCount = 0;
            for (let i = scoreRange.start; i <= scoreRange.end; i++) {
              const score = parseFloat(form[`score${i}`]);
              if (!isNaN(score) && score > 0) {
                formSum += score;
                formCount++;
            ...
            if (formCount > 0) {
              totalSum += formSum / formCount;
              totalCount++;
              console.log(`디버깅중입니다 - 폼 ${idx}: 평균 ${parseFloat((formSum/formCount).toFixed(2))}`);
            }
          });
          
          const result = totalCount > 0 ? parseFloat((totalSum / totalCount).toFixed(2)) : 0;
          console.log(`디버깅중입니다 - 최종 평균: ${result}`);
          return result;
        };

        // Smartphone prevention scores (중독특징이해: 1-3, 핵심증상이해: 4-6, 문제대응방법: 7-9)
        preventEffectResult.smartphone.avgScore1 = calculateScoreAverage(smartphonePreData, {start: 1, end: 3});
        preventEffectResult.smartphone.avgScore2 = calculateScoreAverage(smartphonePreData, {start: 4, end: 6});
        preventEffectResult.smartphone.avgScore3 = calculateScoreAverage(smartphonePreData, {start: 7, end: 9});
        preventEffectResult.smartphone.avgScore4 = calculateScoreAverage(smartphonePostData, {start: 1, end: 3});
        preventEffectResult.smartphone.avgScore5 = calculateScoreAverage(smartphonePostData, {start: 4, end: 6});
        preventEffectResult.smartphone.avgScore6 = calculateScoreAverage(smartphonePostData, {start: 7, end: 9});
        
        console.log('디버깅중입니다 - 스마트폰 최종 결과:', preventEffectResult.smartphone);
      }

      // Process gambling prevention data (prevent_gambling_forms)
      if (preventGamblingFormsData.length > 0) {
        const gamblingPreData = preventGamblingFormsData.filter(form => form.pv === '사전');
        const gamblingPostData = preventGamblingFormsData.filter(form => form.pv === '사후');

        console.log(`디버깅중입니다 - 도박 사전 데이터: ${gamblingPreData.length}개`);
        console.log(`디버깅중입니다 - 도박 사후 데이터: ${gamblingPostData.length}개`);

        const calculateGamblingScoreAverage = (forms, scoreRange) => {
          console.log(`디버깅중입니다 - 도박 점수 계산 중: ${forms.length}개 폼, 점수 범위 ${scoreRange.start}-${scoreRange.end}`);
          if (forms.length === 0) return 0;
          let totalSum = 0;
          let totalCount = 0;
          ...
          forms.forEach((form, idx) => {
            ...
            if (formCount > 0) {
              totalSum += formSum / formCount;
              totalCount++;
              console.log(`디버깅중입니다 - 도박 폼 ${idx}: 평균 ${parseFloat((formSum/formCount).toFixed(2))}`);
            }
          });
          
          const result = totalCount > 0 ? parseFloat((totalSum / totalCount).toFixed(2)) : 0;
          console.log(`디버깅중입니다 - 도박 최종 평균: ${result}`);
          return result;
        };

        // Gambling prevention scores (중독특징이해: 1-3, 핵심증상이해: 4-6, 문제대응방법: 7-9)
        preventEffectResult.gambling.avgScore1 = calculateGamblingScoreAverage(gamblingPreData, {start: 1, end: 3});
        preventEffectResult.gambling.avgScore2 = calculateGamblingScoreAverage(gamblingPreData, {start: 4, end: 6});
        preventEffectResult.gambling.avgScore3 = calculateGamblingScoreAverage(gamblingPreData, {start: 7, end: 9});
        preventEffectResult.gambling.avgScore4 = calculateGamblingScoreAverage(gamblingPostData, {start: 1, end: 3});
        preventEffectResult.gambling.avgScore5 = calculateGamblingScoreAverage(gamblingPostData, {start: 4, end: 6});
        preventEffectResult.gambling.avgScore6 = calculateGamblingScoreAverage(gamblingPostData, {start: 7, end: 9});
        
        console.log('디버깅중입니다 - 도박 최종 결과:', preventEffectResult.gambling);
      }

      // Calculate experience statistics from all prevent forms
      const allPreventFormsForStats = [...preventFormsData, ...preventGamblingFormsData];
      if (allPreventFormsForStats.length > 0) {
        console.log('디버깅중입니다 - 경험 통계 계산 시작');
        
        allPreventFormsForStats.forEach(form => {
          const experience = form.past_stress_experience || '미기재';
          if (experience === '유') {
            preventEffectResult.experienceStats.hasExperience++;
          } else if (experience === '무') {
            preventEffectResult.experienceStats.noExperience++;
          } else {
            preventEffectResult.experienceStats.notSpecified++;
          }
        });
        
        console.log('디버깅중입니다 - 경험 통계 결과:', preventEffectResult.experienceStats);
      }

      // 2. Facility Environment Data (시설환경 만족도)
      console.log('디버깅중입니다 - Facility Environment 데이터 조회 시작');
      const serviceForms = await prisma.serviceForm.findMany({
        where: {
          openday: { gte: startDate, lte: endDate }
        }
      ...
      console.log(`디버깅중입니다 - Service Forms 조회 결과: ${serviceForms.length}개`);

      if (serviceForms.length > 0) {
        // console.log('디버깅중입니다 - 첫 번째 Service Form 데이터:', serviceForms[0]);

        // Define score groups for the new summary
        const scoreGroups = {
          accommodation: [1, 2],
          facility_outdoor: [3, 4, 5, 6, 7, 8, 9, 10],
          operation: [11, 12, 13],
          meal: [14, 15, 16],
          overall: [17]
        };
        ...
        for (const groupName in scoreGroups) {
          const scoreKeys = scoreGroups[groupName].map(n => `score${n}`);
          const scores = serviceForms.flatMap(form => 
            scoreKeys.map(key => parseFloat(form[key]))
          ...
          facilityEnvironmentResult[`${groupName}_avg`] = average;
          console.log(`디버깅중입니다 - ${groupName} 평균: ${average} (${scores.length}개)`);
        }
        
        console.log('디버깅중입니다 - 시설환경 최종 결과:', facilityEnvironmentResult);
      }

      // 3. Healing Effect Data (힐링효과)
      console.log('디버깅중입니다 - Healing Effect 데이터 조회 시작');

      const healingFormsDataTest = await prisma.healingForm.findMany({
      
      });
      console.log("디버깅중입니다 - Healing Forms Test 조회 결과: ", healingFormsDataTest);

      const healingFormsData = await prisma.healingForm.findMany({
        where: {
          openday: { gte: startDate, lte: endDate }
        }
      });

      console.log(`디버깅중입니다 - Healing Forms 조회 결과: ${healingFormsData.length}개`);

      if (healingFormsData.length > 0) {
        console.log('디버깅중입니다 - 첫 번째 Healing Form 데이터:', healingFormsData[0]);
        
        const healingPreData = healingFormsData.filter(form => form.pv === '사전');
        const healingPostData = healingFormsData.filter(form => form.pv === '사후');

        console.log(`디버깅중입니다 - 힐링 사전 데이터: ${healingPreData.length}개`);
        console.log(`디버깅중입니다 - 힐링 사후 데이터: ${healingPostData.length}개`);

        // Calculate healing effect scores for 7 domains
        const calculateHealingDomainAverage = (forms, domainScores) => {
          console.log(`디버깅중입니다 - 힐링 도메인 계산: ${forms.length}개 폼, 점수 인덱스 [${domainScores.join(',')}]`);
          if (forms.length === 0) return 0;
          let totalSum = 0;
          let totalCount = 0;
          ...
          forms.forEach((form, idx) => {
            let domainSum = 0;
            let domainCount = 0;
            domainScores.forEach(scoreIndex => {
              const score = parseFloat(form[`score${scoreIndex}`]);
              if (!isNaN(score) && score > 0) {
                domainSum += score;
                domainCount++;
              }
            ...
            if (domainCount > 0) {
              totalSum += domainSum / domainCount;
              totalCount++;
              console.log(`디버깅중입니다 - 힐링 폼 ${idx}: 도메인 평균 ${parseFloat((domainSum/domainCount).toFixed(2))}`);
            }
          });
          
          const result = totalCount > 0 ? parseFloat((totalSum / totalCount).toFixed(2)) : 0;
          console.log(`디버깅중입니다 - 힐링 도메인 최종 평균: ${result}`);
          return result;
        };

        // Define score ranges for each healing domain (based on healing form structure)
        const healingDomains = [
          [1, 2, 3, 4],    // 욕구충족 - 내면화된 수치심 (score1-4)
          [5, 6, 7, 8],    // 긍정정서 - 문제해결 능력 (score5-8)
          [9, 10, 11, 12], // 자기이해 - 인지적 정서조절 (score9-12)
          [13, 14, 15, 16],// 마음관리기술 - 회복탄력성 (score13-16)
          [17, 18, 19, 20],// 정서능력측면 - 자존감 (score17-20)
          [21],            // 명상측면 - 생활 스트레스 첫 번째 (score21)
          [22]             // 삶의조망측면 - 생활 스트레스 두 번째 (score22)
        ];

        console.log('디버깅중입니다 - 힐링 도메인 정의:', healingDomains);

        // Calculate pre scores
        for (let i = 0; i < healingDomains.length; i++) {
          healingEffectResult[`pre_avg${i + 1}`] = calculateHealingDomainAverage(healingPreData, healingDomains[i]);
        }

        // Calculate post scores
        for (let i = 0; i < healingDomains.length; i++) {
          healingEffectResult[`post_avg${i + 1}`] = calculateHealingDomainAverage(healingPostData, healingDomains[i]);
        }

        // Calculate differences
        for (let i = 1; i <= 7; i++) {
          healingEffectResult[`diff_avg${i}`] = parseFloat((healingEffectResult[`post_avg${i}`] - healingEffectResult[`pre_avg${i}`]).toFixed(2));
        }

        // Calculate experience statistics from healing forms
        console.log('디버깅중입니다 - 힐링 경험 통계 계산 시작');
        
        healingFormsData.forEach(form => {
          const experience = form.past_stress_experience || '미기재';
          if (experience === '유' || experience === '있음') {
            healingEffectResult.experienceStats.hasExperience++;
          } else if (experience === '무' || experience === '없음') {
            healingEffectResult.experienceStats.noExperience++;
          } else {
            healingEffectResult.experienceStats.notSpecified++;
          }
        });
        
        console.log('디버깅중입니다 - 힐링 경험 통계 결과:', healingEffectResult.experienceStats);
        console.log('디버깅중입니다 - 힐링효과 최종 결과:', healingEffectResult);
      }

      console.log('디버깅중입니다 - 모든 새 섹션 데이터 처리 완료');
      console.log('디버깅중입니다 - Prevent Effect 최종:', preventEffectResult);
      console.log('디버깅중입니다 - Facility Environment 최종:', facilityEnvironmentResult);
      console.log('디버깅중입니다 - Healing Effect 최종:', healingEffectResult);

    } catch (error) {
      console.error('디버깅중입니다 - 새 섹션 데이터 계산 중 오류:', error);
    }

    // --- Step 5: Calculate Facility Scores by Agency ---
    const facilityScoresByAgency = {};
    
    try {
      // Get service forms data grouped by agency
      const serviceFormsByAgency = await prisma.serviceForm.findMany({
        where: {
          openday: { gte: startDate, lte: endDate }
        },
        select: {
      ...
      const agencyGroups = {};
      serviceFormsByAgency.forEach(form => {
        if (!form.agency) return;
        
        if (!agencyGroups[form.agency]) {
          agencyGroups[form.agency] = {
            scores: {
        ...
        for (let i = 1; i <= 16; i++) {
          ...
          if (!isNaN(score) && score >= 0) {  // 0점도 유효한 점수로 포함
            agencyGroups[form.agency].scores[`score${i}`].push(score);
          }
        }
      });
      ...
      Object.keys(agencyGroups).forEach(agency => {
        facilityScoresByAgency[agency] = {};
        console.log(`Processing agency: ${agency}`);
        
        for (let i = 1; i <= 16; i++) {
          const scores = agencyGroups[agency].scores[`score${i}`];
          if (scores.length > 0) {
            const average = scores.reduce((sum, score) => sum + score, 0) / scores.length;
            facilityScoresByAgency[agency][`score${i}`] = parseFloat(average.toFixed(2));
            console.log(`  score${i}: ${scores.length} responses, average: ${average.toFixed(2)}`);
          } else {
            facilityScoresByAgency[agency][`score${i}`] = 0;
            console.log(`  score${i}: 0 responses, set to 0`);
          }
        }
      });
      
      console.log('Facility Scores by Agency:', facilityScoresByAgency);
    } catch (error) {
      console.error('Error calculating facility scores by agency:', error);
    }
    ...
    const finalResult = {
      ...
      facilityScoresByAgency: facilityScoresByAgency,
      programEffect: programEffectResult,
      preventEffect: preventEffectResult,
      facilityEnvironment: facilityEnvironmentResult,
      healingEffect: healingEffectResult,
