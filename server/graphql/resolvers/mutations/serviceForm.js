...
const resolvers = {
  ...
  updateProgramForm: async (_, { id, input }) => {
    try {
      ...
      try {
        ...
        console.log(`[updateProgramForm] Successfully updated program form with ID ${parsedId}`);
        return updatedForm;
        
      } catch (updateError) {
        // If update fails because record doesn't exist, create a new one
        if (updateError.code === 'P2025') { // Prisma "Record not found" error
          console.log(`[updateProgramForm] Record with ID ${parsedId} not found, creating new record instead`);
          
          const newForm = await prisma.programForm.create({
            data: {
              agency: input.agency,
              agency_id: input.agency_id || null,
              openday: input.openday,
          ...
          console.log(`[updateProgramForm] Successfully created new program form with ID ${newForm.id}`);
          return newForm;
        } else {
          throw updateError;
        }
      ...
      console.error('Error in updateProgramForm:', error);
    ...
    try {
      ...
      const form = await prisma.counselTherapyForm.create({
        data: {
          ...
          average_usage_time: input.average_usage_time || "",
  ...
  createPreventForm: async (_, { input }) => {
    try {
      ...
      await prisma.preventForm.create({
        data: {
          ...formData,
          past_stress_experience: input.past_stress_experience,
          participation_period: input.participation_period || "",
        }
      });
      return { success: true, message: 'Prevent form created successfully' };
    } catch (error) {
      console.error(error);
      return { success: false, message: 'Failed to create prevent form' };
    }
  },
  ...
  // PreventGambling Form mutations
  createPreventGamblingForm: async (_, { input }) => {
    try {
      const { ...formData } = input;
      await prisma.preventGamblingForm.create({
        data: {
          ...formData,
          past_stress_experience: input.past_stress_experience,
          participation_period: input.participation_period || "",
      ...
      return { success: true, message: 'Prevent gambling form created successfully' };
    } catch (error) {
      console.error(error);
      return { success: false, message: 'Failed to create prevent gambling form' };
    }
  },
  
  updatePreventGamblingForm: async (_, { id, input }) => {
    try {
      // Get existing form to verify it exists
      const existingForm = await prisma.preventGamblingForm.findUnique({
        where: { id: parseInt(id) }
      });
      
      if (!existingForm) {
        throw new Error(`Prevent gambling form with ID ${id} not found`);
      }
      
      // Update the form with all fields directly
      const updatedForm = await prisma.preventGamblingForm.update({
        where: { id: parseInt(id) },
        data: {
          agency: input.agency,
          agency_id: input.agency_id || null,
          ...
          past_stress_experience: input.past_stress_experience || "",
          participation_period: input.participation_period || "",
          prevent_gambling_seq: input.prevent_gambling_seq || null,
          sex: input.sex || "미기재",
          age: input.age || "",
          residence: input.residence || "미기재",
          job: input.job || "",
      ...
      console.error('Error updating prevent gambling form:', error);
      throw new Error('Failed to update prevent gambling form');
    }
  },
  
  deletePreventGamblingForm: async (_, { id }) => {
    try {
      // Prisma will cascade delete the entries due to the relation
      await prisma.preventGamblingForm.delete({
        where: { id: parseInt(id) }
      });
      
      return true;
    } catch (error) {
      console.error('Error deleting prevent gambling form:', error);
      throw new Error('Failed to delete prevent gambling form');
    }
  },
