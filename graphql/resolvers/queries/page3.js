      ...
      const result = await Promise.all(rooms.map(async room => {
        ...
        const roomManageRecords = await prisma.roomManage.findMany({
          where: {
            ...
            OR: [
              // Check-in date is within the range
              {
                check_in_date: {
                  gte: requestStart.toDate(),
                  lt: requestEnd.toDate()
                }
              },
              // Check-out date is within the range
              {
                check_out_date: {
                  gt: requestStart.toDate(),
                  lte: requestEnd.toDate()
                }
              },
              // Start and end dates span over the room's booking
              {
                AND: [
                  { check_in_date: { lte: requestStart.toDate() } },
                  { check_out_date: { gte: requestEnd.toDate() } }
                ]
              }
            ]
          }
        ...
        for (const page3 of page3Records) {
          if (!page3.room_selections) continue;
          ...
          for (const selection of roomSelections) {
            if (selection.room_id == room.id) {
              ...
              // 1. 요청 시작일이 예약 기간 내에 있는 경우
              // 2. 요청 종료일이 예약 기간 내에 있는 경우
              // 3. 요청 기간이 예약 기간을 포함하는 경우
              // 4. 예약 기간이 요청 기간을 포함하는 경우
              const isOverlapping = 
                (requestStart.isSameOrAfter(reserveStart) && requestStart.isBefore(reserveEnd)) || // 시작일 겹침
                (requestEnd.isAfter(reserveStart) && requestEnd.isSameOrBefore(reserveEnd)) || // 종료일 겹침
                (requestStart.isBefore(reserveStart) && requestEnd.isAfter(reserveEnd)) || // 요청이 예약을 감싸는 경우
                (reserveStart.isBefore(requestStart) && reserveEnd.isAfter(requestEnd)); // 예약이 요청을 감싸는 경우
