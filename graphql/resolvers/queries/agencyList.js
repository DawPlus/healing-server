...
const getPreventList = async (_, { type, agency, agency_id, openday, endday, keywords }) => {
  try {
    ...
    // Get prevent form data (no more entries relation)
    const preventForms = await prisma.preventForm.findMany({
      where,
      orderBy: [
        {
    ...
    return filteredResults.map(item => {
      ...
      const sum4List = [item.SCORE11, item.SCORE12];
      const sum5List = [item.SCORE13, item.SCORE14, item.SCORE15, item.SCORE16, item.SCORE17];
      const sum6List = [item.SCORE18, item.SCORE19, item.SCORE20];
      ...
      return { ...item, sum1, sum2, sum3, sum4, sum5, sum6 };
    });
  } catch (error) {
